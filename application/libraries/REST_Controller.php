...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
       ...
       //User passed language
        $rest_language_supported = $this->config->item('rest_language_supported') ;
        $ContentLanguage = $this->input->get_request_header("language",TRUE);
       
        // Get the language       
        $language = $this->config->item('rest_language');
        if ($language === NULL && !in_array($ContentLanguage, $rest_language_supported))
        {
            $language = 'english';
        }
        else
    ...
    {
        $data = trim($this->common->AES_encrypt(json_encode($data)));   
